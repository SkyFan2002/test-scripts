...
# Fetch all tables from the system (excluding the 'system' database)
tables=$(echo "SELECT catalog, database, name FROM system.tables WHERE database != 'system';" | bendsql)
if [ $? -ne 0 ]; then
...
# Final summary
log_message "Scan complete. Successfully scanned $success_count tables, failed to scan $failed_count tables."
